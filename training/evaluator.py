...
def evaluate(prediction, ground_truth, mask, report=False):
    ...
    #         if(prediction[i][j] > 0):
    #             up_ratio_pre = up_ratio_pre + prediction[i][j]
    #             up_ratio_tru = up_ratio_tru + ground_truth[i][j]
    #         else:
    #             down_ratio_pre = down_ratio_pre + prediction[i][j]
    #             down_ratio_tru = down_ratio_tru + ground_truth[i][j]


    performance['mrrt'] = mrr_top / (prediction.shape[1] - all_miss_days_top)
    # performance['up_ratio'] = abs(up_ratio_pre/up_ratio_tru)
    # performance['down_ratio'] = abs(down_ratio_pre/down_ratio_tru)
