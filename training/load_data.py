...
def load_EOD_data(data_path, market_name, tickers,  steps=1):
    ...
    for index, ticker in enumerate(tickers):
        ...
        if index == 0:
            print('single EOD data shape:', single_EOD.shape)#(1245,6)
            eod_data = np.zeros([len(tickers), single_EOD.shape[0],
                                 single_EOD.shape[1] - 1], dtype=np.float32)#（1026,1245,5）
            masks = np.ones([len(tickers), single_EOD.shape[0]],
                            dtype=np.float32)#(1026, 1245)
        ...
        for row in range(single_EOD.shape[0]):
            if abs(single_EOD[row][-1] + 1234) < 1e-8:#空样本
                masks[index][row] = 0.0
                ...
                ground_truth[index][row] = \
                    ...
                    single_EOD[row - steps][-1]#真实值算增长率
...
def load_graph_relation_data(relation_file, lap=False):
    ...
    print('relation encoding shape:', relation_encoding.shape)#(1026, 1026, 43)
    ...
    # GAT
    if lap:
        return ajacent, np.identity(ajacent.shape[0], dtype=float) - np.dot(
            np.dot(deg_neg_half_power, ajacent), deg_neg_half_power)
    else:
        return ajacent, np.dot(np.dot(deg_neg_half_power, ajacent), deg_neg_half_power)
...
def load_relation_data(relation_file):
    ...
    mask_flags = np.equal(np.zeros(rel_shape, dtype=int),
                          np.sum(relation_encoding, axis=2))
    mask = np.where(mask_flags, np.ones(rel_shape) * -1e9, np.zeros(rel_shape))# (1026, 1026)
